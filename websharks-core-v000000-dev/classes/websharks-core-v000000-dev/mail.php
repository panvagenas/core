...
namespace websharks_core_v000000_dev
{
	...
	class mail extends framework
	{
		 ...
		 *
		 * @TODO Update this routine to use ``check_extension_arg_types()``.
		 *
		...
		public function send($mail)
		{
			...
			// Check email message subject line.

			if(!$this->©string->is_not_empty($mail['subject']))
				throw $this->©exception(
					__METHOD__.'#subject_missing', compact('mail'),
					$this->i18n('Email failure. Missing and/or invalid `subject` value.')
				);

			// Check email message body.

			if(!$this->©string->is_not_empty($mail['message']))
				throw $this->©exception(
					__METHOD__.'#message_missing', compact('mail'),
					$this->i18n('Email failure. Missing and/or invalid `message` value.')
				);

			// Check sender i.e. `from_(addr|name)`.

			if(!$this->©string->is_not_empty($mail['from_addr']))
				throw $this->©exception(
					__METHOD__.'#from_addr_missing', compact('mail'),
					$this->i18n('Email failure. Missing and/or invalid `from_addr` value.')
				);
			$this->©string->isset_or($mail['from_name'], '', TRUE); // Optional name.

			// Check recipients i.e. `recipients`.

			if( // This can be a string, or an array.
				!isset($mail['recipients']) // Make sure it's set.
				|| !($mail['recipients'] = $this->parse_emails_deep($mail['recipients']))
			) // The `recipients` are always parsed into an array here.
				throw $this->©exception(
			...
			foreach($mail['attachments'] as &$_attachment)
			{
				...
				if(!$this->©string->is_not_empty($_attachment['path']))
					throw $this->©exception(
						__METHOD__.'#attachment_path_missing', compact('_attachment'),
