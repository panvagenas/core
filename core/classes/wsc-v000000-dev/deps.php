...
if(!class_exists('deps_wsc_v000000_dev'))
{
	...
	if(!class_exists('wsc_v000000_dev'))
	{
	 ...
	 * @since 120318
	 *
	 * @see deps_x_wsc_v000000_dev
	 */
	final class deps_wsc_v000000_dev // Static properties/methods only please.
	{
		# --------------------------------------------------------------------------------------------------------------------------------
		...
		public static function initialize()
		{
			 ...
			 */
			if(!class_exists('wsc__deps') && function_exists('class_alias') /* PHP v5.3+ only. */)
				class_alias(__CLASS__, 'wsc__deps');
		 ...
		 *
		 * @inheritdoc deps_x_wsc_v000000_dev::check()
		 * @see deps_x_wsc_v000000_dev::check()
		 */
		                             ...
		                             $check_last_ok = TRUE, $maybe_display_wp_admin_notices = TRUE)
		{
			...
			) throw new exception(sprintf(wsc_v000000_dev::__('Invalid arguments: `%1$s`'), print_r(func_get_args(), TRUE)));

			if(apply_filters('wsc__deps__check_disable', FALSE))
			...
			if(!class_exists('deps_x_wsc_v000000_dev'))
				require_once dirname(__FILE__).'/deps-x.php';

			$x             = new deps_x_wsc_v000000_dev();
		...
		public static function ___uninstall___($confirmation = FALSE)
		{
			if(!is_bool($confirmation))
				throw new exception( // Fail here; detected invalid arguments.
					sprintf(wsc_v000000_dev::__('Invalid arguments: `%1$s`'),
	...
	deps_wsc_v000000_dev::initialize(); // Also creates class alias.
}