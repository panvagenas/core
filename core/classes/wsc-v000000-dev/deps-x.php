...
${__FILE__}['is_in_stand_alone_mode'] = // Are we in stand-alone mode?
	(class_exists('deps_x_stand_alone_wsc_v000000_dev') && basename(__FILE__) !== 'deps-x.php'
 ...
 * @since 120318
 */
final class deps_x_wsc_v000000_dev #!stand-alone!# // MUST remain PHP v5.2 compatible.
{
	# --------------------------------------------------------------------------------------------------------------------------------------
	# Public properties.
	# --------------------------------------------------------------------------------------------------------------------------------------
	 ...
	 * @var string A generic default value.
	 */
	public $default_plugin_dir_names = 'wsc-v000000-dev';
	...
	public static function initialize()
	{
		 ...
		 */
		if(!$GLOBALS[__FILE__]['is_in_stand_alone_mode'] && !class_exists('wsc__deps_x') && function_exists('class_alias'))
			class_alias('deps_x_wsc_v000000_dev', 'wsc__deps_x');

		if($GLOBALS[__FILE__]['is_in_stand_alone_mode'] && !class_exists('wsc__deps_x_stand_alone') && function_exists('class_alias'))
			class_alias('deps_x_stand_alone_wsc_v000000_dev', 'wsc__deps_x_stand_alone');
	 ...
	 *    as class: `deps_x_stand_alone_wsc_v000000_dev`.
	 *
	 ...
	 *
	 * @see deps_wsc_v000000_dev::check()
	 */
	                      ...
	                      $check_last_ok = TRUE, $maybe_display_wp_admin_notices = TRUE)
	{
		...
		$_g                    = ($is_wp_loaded && !empty($_GET['wsc__deps'])) ? stripslashes_deep($_GET['wsc__deps']) : array();
		...
		if($is_wp_loaded && !$is_stand_alone && apply_filters('wsc__deps__check_disable', FALSE))
		...
		if($is_wp_loaded && !$is_stand_alone && $check_last_ok
		   && is_array($last_ok = get_option('wsc__deps__last_ok'))
		   && isset($last_ok['wsc_v000000_dev'], $last_ok['time'], $last_ok['php_version'], $last_ok['wp_version'])
		...
		$_curl_fopen_localhost_test_url .= '/?wsc__deps[localhost_test_url]=1';
		...
		{
			if(!is_array($_dismissals = get_option('wsc__deps__notice__dismissals')))
				add_option('wsc__deps__notice__dismissals', ($_dismissals = array()), '', 'no');
			...
			foreach($issues as $_key => $_issue) // Loop over each issue.
			{
				...
				if($is_auto_fix && isset($_issue['data']['auto_fix'], $_g['auto_fix']) && $_issue['data']['auto_fix'] === $_g['auto_fix'])
				{
					...
					{
						$_retry = remove_query_arg('wsc__deps', (string)$_SERVER['REQUEST_URI']);
			...
			if($_dismissals_require_update) // Update now.
			{
				...
				update_option('wsc__deps__notice__dismissals', $_dismissals);
			}
		...
		else if($is_wp_loaded && ($issues || !get_option('wsc__deps__last_ok')))
		{
			update_option(
				'wsc__deps__last_ok', array(
	 ...
	 *    `deps_x_stand_alone_wsc_v000000_dev`.
	 *
	 ...
	 * @return string {@inheritdoc}
	 *
	 * @see wsc_v000000_dev\env::apache_version()
	 * @inheritdoc wsc_v000000_dev\env::apache_version()
	 */
	 ...
	 *
	 * @see wsc_v000000_dev\functions::is_possible()
	 * @inheritdoc wsc_v000000_dev\functions::is_possible()
	 */
	 ...
	 *
	 * @see wsc_v000000_dev\functions::disabled()
	 * @inheritdoc wsc_v000000_dev\functions::disabled()
	 */
	 ...
	 *
	 * @see wsc_v000000_dev\files::abbr_bytes()
	 * @inheritdoc wsc_v000000_dev\files::abbr_bytes()
	 */
	 ...
	 * @return string {@inheritdoc}
	 *
	 * @see wsc_v000000_dev::n_dir_deps()
	 * @inheritdoc wsc_v000000_dev::n_dir_deps()
	 */
	 ...
	 * @return string {@inheritdoc}
	 *
	 * @see wsc_v000000_dev::n_dir_seps_up()
	 * @inheritdoc wsc_v000000_dev::n_dir_seps_up()
	 */
	 ...
	 * @return string {@inheritdoc}
	 *
	 * @see wsc_v000000_dev::wp_load()
	 * @inheritdoc wsc_v000000_dev::wp_load()
	 */
	 ...
	 * @return string {@inheritdoc}
	 *
	 * @see wsc_v000000_dev::locate()
	 * @inheritdoc wsc_v000000_dev::locate()
	 */
	 ...
	 * @return boolean {@inheritdoc}
	 *
	 * @see wsc_v000000_dev\env::is_localhost()
	 * @inheritdoc wsc_v000000_dev\env::is_localhost()
	 */
	 ...
	 * @return boolean {@inheritdoc}
	 *
	 * @see wsc_v000000_dev\env::is_cli()
	 * @inheritdoc wsc_v000000_dev\env::is_cli()
	 */
	 ...
	 * @return boolean {@inheritdoc}
	 *
	 * @see wsc_v000000_dev\env::ob_end_clean()
	 * @inheritdoc wsc_v000000_dev\env::ob_end_clean()
	 */
	...
	public function ___uninstall___($confirmation = FALSE)
	{
		...
		delete_option('wsc__deps__last_ok');
		delete_option('wsc__deps__notice__dismissals');
	 ...
	 * @return string {@inheritdoc}
	 *
	 * @see wsc_v000000_dev::__()
	 * @inheritdoc wsc_v000000_dev::__()
	 */
	public function __($string)
	{
		$core_ns_stub_with_dashes = 'wsc'; // Core namespace stub w/ dashes.
	 ...
	 * @return string {@inheritdoc}
	 *
	 * @see wsc_v000000_dev::_x()
	 * @inheritdoc wsc_v000000_dev::_x()
	 */
	 ...
	 *
	 * @see wsc_v000000_dev::$regex_valid_dir_file_stream_wrapper
	 * @inheritdoc wsc_v000000_dev::$regex_valid_dir_file_stream_wrapper
	 */
	 ...
	 *
	 * @see wsc_v000000_dev::$regex_valid_win_drive_letter
	 * @inheritdoc wsc_v000000_dev::$regex_valid_win_drive_letter
	 */
...
if(${__FILE__}['is_in_stand_alone_mode']) // Running in Stand-Alone mode?
	deps_x_stand_alone_wsc_v000000_dev::initialize();
else deps_x_wsc_v000000_dev::initialize();
...
if(${__FILE__}['is_in_stand_alone_mode']) // Running in Stand-Alone mode?
{
	...
	define('___STAND_ALONE__PLUGIN_DIR_NAMES', 'wsc-v000000-dev'); #!stand-alone-plugin-dir-names!#

	$deps_x_stand_alone_wsc_v000000_dev = new deps_x_stand_alone_wsc_v000000_dev();

	if(!defined('WPINC') && $deps_x_stand_alone_wsc_v000000_dev->wp_load())
		require_once $deps_x_stand_alone_wsc_v000000_dev->wp_load(TRUE);

	$deps_x_stand_alone_wsc_v000000_dev->check();
}